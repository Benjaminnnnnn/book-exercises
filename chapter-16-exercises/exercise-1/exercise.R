...
# clarity.
ggplot(diamonds_sample, aes(x = carat, y = price, color = carat)) + geom_point()
...
ggplot(diamonds_sample) + geom_point(mapping = aes(y=price, x=carat), color = "blue")

# Draw a scatter plot for `diamonds_sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.
ggplot(diamonds_sample, aes(x=carat, y=price, shape=cut, color = carat)) + geom_point()
...
ggplot(diamonds_sample, aes(x=carat, y=cut, size=price, color=carat)) + geom_point() + scale_fill_gradient2(low = "#132B43", high = "#56B1F7")
...
ggplot(diamonds_sample) + geom_smooth(mapping=aes(x= carat, y=price, color=cut))
...
ggplot(diamonds_sample, mapping=aes(x=cut, y=price)) + geom_col()

# Add an aesthetic property that will _fill_ each bar geometry based on the 
# `clarity` of the diamonds. 
# What kind of chart do you get?
ggplot(diamonds_sample) + geom_col(mapping=aes(x=cut, y=price, fill=carat))
...
diamonds_sample %>% group_by(carat) %>% ggplot(alpha=0.3) + geom_smooth(aes(x=carat, y=price)) 
